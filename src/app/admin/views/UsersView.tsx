...
export default function UsersView() {
  ...
  const { success: successMsg, error: errorMsg, showSuccess, showError } = useFeedback();
  const [activeTab, setActiveTab] = useState<'overview' | 'challenge' | 'finance' | 'referrals'>('overview');
              ...
              <div className="p-24 border-b border-white/5 bg-white/[0.01]">
                ...
                <div className="flex gap-2 p-4 bg-white/5 rounded-lg w-full">
                  {[
                    ...
                    { id: 'challenge', label: 'HISTORY', icon: Sword },
                    { id: 'finance', label: 'WALLET', icon: Wallet },
                    { id: 'referrals', label: 'NETWORK', icon: TrendingUp },
                  ].map(tab => (
                    <button 
                      key={tab.id}
                      onClick={() => setActiveTab(tab.id as 'overview' | 'challenge' | 'finance' | 'referrals')} 
